...
* `usage.py` — `SourceUsageAnalyzer.find_usage(repo_path, package_name, ecosystem, component_paths) -> UsageEvidence(package_name, ecosystem, import_names, references[UsageReference(file,line,snippet,kind)], files, components, truncated)`. Python: import names from a built-in mapping for common packages (`pyyaml→yaml`, `pillow→PIL`, `beautifulsoup4→bs4`, `scikit-learn→sklearn`, `python-dateutil→dateutil`, `python-dotenv→dotenv`, `msgpack-python→msgpack`, `psycopg2-binary→psycopg2`, `opencv-python→cv2`, `attrs→attr`, `pyjwt→jwt`, `markupsafe→markupsafe`, `jinja2→jinja2`, `flask-*→flask_*`, ...) plus heuristics (`-`→`_`, strip `python-`/`py` prefixes, lowercase); regex over `*.py` (`^\s*(import|from)\s+<name>(\.|\s|$)`). JS: `require('<pkg>')`, `import ... from '<pkg>'`, `import '<pkg>'`, dynamic `import('<pkg>')` incl. sub-paths (`<pkg>/x`) over `*.js *.jsx *.ts *.tsx *.mjs *.cjs *.vue`. Also flags `kind="config"` references in dependency files. Ignore dirs as in §4.1; cap 5000 files / 100 references (set `truncated`); eligible files larger than 1 MB or unreadable are listed in `skipped_files` (+ `skipped_files_total`) and also set `truncated` (audit V2-03); `analysis_depth = "direct-import-scan"` states what the evidence covers. `usage_verdict(evidence, scope) -> UsageVerdict(kind, message)` classifies "no usage" as `beyond-depth` (transitive), `scan-incomplete`, `unknown-scope` or `no-direct-import` — exposed as `FindingDetail.usage_verdict`, in the agent context (`UsageContext.verdict`) and in report sections 5/6 (audit V2-02). `components` = component paths whose directory contains a referencing file (root `.` for top-level files).
* `risk.py` — `provisional_risk_from_severity(severity) -> RiskLevel`, `floor_risk_level(judged, provisional) -> RiskLevel` (the AI may raise a level, never lower it below the severity-derived one; UNKNOWN keeps the provisional level — audit V2-01), `effective_risk_level(finding)` (stored level floored on read, so rows written before the floor cannot lower an analysis), `aggregate_overall_risk(findings) -> RiskLevel` (max of effective levels), `risk_rank(level) -> int`.
...
* `modifiers.py` — `DependencyFileModifier(ABC).apply(workspace, relative_file, package_name, current_version, new_version) -> FileChange(file, before, after, diff, line_number, notes)` built through `FileChange.build(...)`, which redacts manifest credentials (`core/redaction.py`) from the reported before/after/diff and adds a note — the file written to the working copy keeps the real content (audit V2-04); `RequirementsTxtModifier` (rewrites the matching requirement line preserving extras/markers/comments; `==new`), `PackageJsonModifier` (rewrites the spec in whichever dependency section holds the package, preserving the range operator style: `^1.2.3`→`^new`, `~`→`~new`, exact→exact; preserves indentation/formatting via `json.dumps(indent=detected)` + trailing newline); `get_modifier(source_file)`.
...
* `docker_provider.py` — `DockerSandboxProvider(settings, client=None)`: image by ecosystem (`docker_python_image` / `docker_node_image`); creates a container whose own command is only a keep-alive (`sleep infinity`) running as the non-root `docker_sandbox_user` (default `65534:65534`) with `read_only=True`, in-memory `tmpfs` mounts for `/workspace` (owned by that user, `docker_workspace_tmpfs_size`) and `/tmp`, `network_mode=docker_sandbox_network` (`bridge` by default — package registries must be reachable — or `none`), `mem_limit`, `nano_cpus`, `pids_limit=512`, `security_opt=["no-new-privileges"]`, `cap_drop=["ALL"]`, working dir `/workspace`; starts it and streams the workspace tar (entries owned by the sandbox user, ignored dirs excluded) through `tar -x` on an exec's stdin (`put_archive` is refused for read-only containers); then runs **each step as its own `docker exec`** (`build`: `python -m venv /workspace/.venv && …/pip install -r <file>` / `npm install`; `tests`: `…/.venv/bin/python -m pytest -q` / `npm test` when a real test suite exists, else `SKIPPED` with a reason; `HOME`, pip and npm caches point into the workspace tmpfs). The exit code of every step is the one reported by the Docker daemon for that exec — nothing printed inside the container can influence the verdict. A host-side deadline of `docker_timeout` seconds covers all steps; on expiry the container is killed and the interrupted step and all later steps are `UNKNOWN` regardless of what the exec returned (`timed_out=True`). The combined log is assembled on the host (`### step <name>: <command>` headers, raw step output, `[exit N in Ts]` / `[killed: …]` trailers). `package-lock.json` is read back for npm through an exec (`get_archive` cannot read the in-memory workspace), the container is always removed (`finally`). `build_plan(request) -> SandboxPlan(build_command, test_command, tests_skipped_reason)` is a pure function (unit-tested).
...
* The Docker sandbox copies the workspace into the container (`put_archive`); no host paths
  ...
  socket mounted.
* The sandbox runs as a non-root user on a read-only root filesystem with in-memory
  `/workspace` and `/tmp`; the workspace is streamed in via an exec'd `tar` and artefacts
  are read back via exec. Network stays on by default (installs need the registries) and can
  be switched off with `DOCKER_SANDBOX_NETWORK=none` for vendored projects (audit F-08).
* `RepositoryService.delete()` removes the repository's remediation working copies,
  validation logs and reports together with the rows (paths derived from ids, confined to
  the workspace) (audit F-07).
* Sandbox verdicts never come from container output: each step is a separate `docker exec`
  whose exit code is read from the daemon, and a host-side deadline turns any unfinished
  step into `UNKNOWN` (the earlier stdout-marker design was forgeable — audit F-01).
* A copied `.git` is untrusted data (audit F-02): `repository/git_safety.py` removes
  `.git/hooks`, rewrites `.git/config` to an allow-list (core basics, `user.name/email`,
  `remote.*.url/fetch`, `branch.*.remote/merge`) and severs `.git` symlinks / `gitdir:`
  pointers so commits can never land in the original repository; every git command
  Sentinel Chain runs in a working copy carries `SAFE_GIT_OPTIONS`
  (`core.hooksPath=/dev/null`, `core.fsmonitor=false`, empty `sshCommand`/`gitProxy`/
  `credential.helper`, HTTPS-only transport) and commits use `--no-verify`.
