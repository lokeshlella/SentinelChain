...
* **Operations** — every long stage (ingestion, analysis, remediation, on-demand AI,
  validation) is a `202` background job with status polling (audit F-05); heartbeats and a
  stale-job watchdog (audit F-06); workspace-relative path storage so rows are not tied to one
  instance's filesystem (audit F-10); `503 DatabaseUnavailable` when PostgreSQL is down
  (audit F-04); health endpoint for every backing service, stage-prefixed logs, startup sweep
